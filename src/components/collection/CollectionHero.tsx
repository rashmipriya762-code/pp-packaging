    ...
    <>
      <section className="bg-[#FAF7F2] pt-4 pb-3 md:pt-6 md:pb-4 px-4 md:px-6 lg:px-8">
        <div className="max-w-[1440px] mx-auto">
          <div className={`${isBlue ? 'bg-[#16323D]' : 'bg-[#F4ECE0]'} rounded-tl-[2rem] md:rounded-tl-[3rem] rounded-bl-[2rem] md:rounded-bl-[3rem] rounded-tr-[1.5rem] md:rounded-tr-[2rem] rounded-br-[0.5rem] md:rounded-br-[1rem] flex flex-col lg:flex-row items-stretch overflow-hidden shadow-sm w-full lg:max-h-[580px]`}>
            ...
            <div className="w-full lg:w-[45%] flex flex-col items-start px-6 py-8 md:px-10 md:py-10 lg:px-14 lg:py-12 shrink-0 justify-center">
              <p className={`eyebrow ${isBlue ? 'text-gold-400' : 'text-gold-600'} mb-4 font-bold tracking-widest text-xs uppercase`}>
                {eyebrow}
              </p>
              
              <h1 className={`font-display text-4xl md:text-5xl lg:text-[3.5rem] leading-[1.08] tracking-tight ${isBlue ? 'text-white' : 'text-[#16323D]'} mb-4 lg:mb-6`}>
              ...
              <p className={`font-body ${isBlue ? 'text-white/80' : 'text-[#5C6B72]'} text-sm md:text-base leading-relaxed mb-6 lg:mb-8 max-w-md`}>
                {description}
              </p>
